...
/*The stack here implemented has all the stack functions such as push(),pop()
	isFull(),isEmpty() and another  extra function gitMax() which return the
	max value present in the stack*/
...
/*A class that support all the functionalitis of Stack and holds an additional
	function getMax() which return the maximum value from the stack at any time.
...
void SpecialStack::push(int x)
{
	...
	else
	{
		...
		auxiliarystack.push(x);
		if(x > y)//Check if the inserted element is greater than remaining elements
...
/*SpecialStack's method to get the maximum value of it*/
